...
export interface ChartSelection {
    startX: number; endX: number; startIndex: number; endIndex: number; active: boolean;
}
...
export interface ChartContextValue {
    ...
    clearSelection?: () => void;
    barScale?: ScaleBandType<string>; bandWidth?: number;
    hoveredBarIndex?: number | null;
    setHoveredBarIndex?: (index: number | null) => void;
    barXAccessor?: (d: Record<string, unknown>) => string;
    orientation?: "vertical" | "horizontal";
    stacked?: boolean; stackOffsets?: Map<number, Map<string, number>>;
}
...
function useChartInteraction({ xScale, yScale, data, lines, margin, xAccessor, bisectDate, canInteract }: UseChartInteractionParams): ChartInteractionResult {
    ...
    const resolveTooltipFromX = useCallback((pixelX: number): TooltipData | null => {
        ...
        if (d1) {
            if (x0.getTime() - xAccessor(d0).getTime() > xAccessor(d1).getTime() - x0.getTime()) { d = d1; finalIndex = index; }
        }
    ...
    const getChartX = useCallback((event: React.MouseEvent<SVGGElement> | React.TouchEvent<SVGGElement>, touchIndex = 0): number | null => {
        ...
        if ("touches" in event) {
            const touch = event.touches[touchIndex]; if (!touch) return null;
    ...
    const handleMouseMove = useCallback((event: React.MouseEvent<SVGGElement>) => {
        ...
        if (isDraggingRef.current) {
            ...
            setSelection({ startX, endX, startIndex: resolveIndexFromX(startX), endIndex: resolveIndexFromX(endX), active: true }); return;
        }
