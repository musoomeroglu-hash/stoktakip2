...
export default function SalesPage({ sales, repairs, phoneSales, products, setSales, onRefresh, customers, setCustomers }: SalesPageProps) {
    ...
    const handleBarcodeKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter') {
            ...
            const prod = products.find(p => p.barcode === val || p.id === val);
            if (prod) {
                const existingIdx = saleItems.findIndex(item => item.productId === prod.id);
                if (existingIdx > -1) {
                    const updated = [...saleItems];
                    updated[existingIdx].quantity += 1;
                    setSaleItems(updated);
                    showToast(`${prod.name} miktarı artırıldı: ${updated[existingIdx].quantity}`);
                } else {
                ...
                showToast(`Ürün bulunamadı: ${val}`, 'error');
                setBarcodeInput('');
                barcodeInputRef.current?.focus();
            }
        }
    ...
    useEffect(() => {
        if (showPosScanner) {
            const startScanner = async () => {
                try {
                    const scanner = new Html5Qrcode('pos-barcode-scanner-container');
            ...
            return () => {
                ...
                if (posScannerRef.current) {
                    posScannerRef.current.stop().then(() => posScannerRef.current?.clear()).catch(() => {});
                    posScannerRef.current = null;
                }
            };
        }
    }, [showPosScanner, products, saleItems, showToast]);
                    ...
                    <div className="bg-surface-dark border border-slate-700 rounded-2xl w-[95vw] md:w-full md:max-w-3xl max-h-[90vh] overflow-y-auto animate-fade-in" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="bg-slate-800/40 p-4 rounded-xl border border-slate-700/50 space-y-3">
                                <label className="block text-sm font-medium text-slate-300">Barkod / IMEI Okutarak Ekle</label>
                                <div className="flex gap-2">
                                    <div className="relative flex-1">
                                        <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-lg">barcode</span>
                                        <input
                                            type="text"
                                            value={barcodeInput}
                                            onChange={e => setBarcodeInput(e.target.value)}
                                            onKeyDown={handleBarcodeKeyDown}
                                            placeholder="Barkod okutun veya IMEI girin..."
                                            className="w-full bg-slate-800 border border-slate-700 rounded-lg py-2 pl-10 pr-4 text-sm text-white placeholder:text-slate-500 focus:border-primary outline-none"
                                    ...
                                    <button
                                        ...
                                        className="px-4 py-2 bg-amber-500/10 border border-amber-500/30 text-amber-400 hover:bg-amber-500/20 rounded-lg text-sm font-bold flex items-center gap-1 transition-all btn-press"
                                    >
                    ...
                    <div className="bg-surface-dark border border-slate-700 rounded-2xl w-[95vw] md:w-full md:max-w-md animate-fade-in" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between p-4 border-b border-slate-700">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="text-lg font-bold text-white">Kamera ile Barkod Okut</h3>
                            </div>
                            <button onClick={() => {
                                if (posScannerRef.current) {
                                    posScannerRef.current.stop().then(() => posScannerRef.current?.clear()).catch(() => {});
                        ...
                        <div className="p-4">
                            <p className="text-sm text-slate-400 mb-3">Barkodu kameraya gösterin, otomatik sepete eklenecek</p>
                            <div id="pos-barcode-scanner-container" className="rounded-lg overflow-hidden bg-black" style={{ minHeight: 280 }}></div>
                        </div>
                    </div>
                </div>
            )}
